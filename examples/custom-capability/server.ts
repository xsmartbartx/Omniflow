...
const CUSTOMERS: Record<string, Output> = {
  c_1: { name: 'Acme Ltd', tier: 'pro' },
  c_2: { name: 'Globex', tier: 'free' },
};
...
export const lookupCustomer = defineCapability<Input, Output>({
  declaration: {
    ...
    inputSchema: {
      ...
      properties: { customerId: { type: 'string', pattern: '^c_[0-9]+$' } },
    },
    outputSchema: {
      type: 'object',
      required: ['name', 'tier'],
    ...
    failureModes: [
      { code: 'CUSTOMER_NOT_FOUND', class: 'business', retryable: false, description: 'No customer has that id' },
    ],
  ...
  async execute(ctx, input) {
    ...
    if (!found)
      throw new CapabilityError('CUSTOMER_NOT_FOUND', `No customer ${input.customerId}`, {
        errorClass: 'business',
        retryable: false,
      });
    return found;
