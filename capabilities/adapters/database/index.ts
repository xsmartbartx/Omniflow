...
export interface DbDriver {
  ...
  command(sql: string, params: SqlParam[], o: { idempotencyKey?: string | undefined; timeoutMs: number }): Promise<CommandResult>;
  close(): Promise<void>;
}
export type SqlParam = string | number | boolean | null;

export interface PgLike {
  connect(): Promise<{ query(q: unknown, v?: unknown[]): Promise<{ rows: Row[]; rowCount: number | null }>; release(): void }>;
  end(): Promise<void>;
}
export type PgFactory = (connectionString: string) => PgLike;

const defaultPgFactory: PgFactory = (connectionString) => new pg.Pool({ connectionString, max: 4, idleTimeoutMillis: 30_000, connectionTimeoutMillis: 10_000 }) as unknown as PgLike;

const IDEMPOTENCY_DDL = 'CREATE TABLE IF NOT EXISTS omniflow_idempotency (key TEXT PRIMARY KEY, created_at TEXT NOT NULL)';

function clean(row: Row): Row {
  const out: Row = {};
  for (const [k, v] of Object.entries(row)) {
    out[k] = typeof v === 'bigint' ? (Number.isSafeInteger(Number(v)) ? Number(v) : v.toString()) : v instanceof Date ? v.toISOString() : v instanceof Uint8Array ? Buffer.from(v).toString('base64') : v;
  }
  return out;
}
      ...
      try {
        ...
        try {
          if (idempotencyKey) {
            ...
            const res = db.prepare('INSERT OR IGNORE INTO omniflow_idempotency (key, created_at) VALUES (?, ?)').run(idempotencyKey, new Date().toISOString());
  ...
  return {
    async query(sql, params, { maxRows, timeoutMs }) {
      ...
      try {
        ...
        return { rows: res.rows.slice(0, maxRows).map(clean), rowCount: res.rows.length, truncated: res.rows.length > maxRows };
      } catch (e) {
        try {
          await client.query('ROLLBACK');
        } catch {
    ...
    async command(sql, params, { idempotencyKey, timeoutMs }) {
      ...
      try {
        ...
        if (idempotencyKey) {
          ...
          const ins = await client.query('INSERT INTO omniflow_idempotency (key, created_at) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING', [idempotencyKey, new Date().toISOString()]);
          if ((ins.rowCount ?? 0) === 0) {
            await client.query('ROLLBACK');
            return { rows: [], rowCount: 0, duplicate: true };
          }
...
function classify(e: unknown): CapabilityError {
  ...
  if (['ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT', '57P01', '57P03', '08006', '08001', '53300', '40001', '40P01', '55P03', '57014'].includes(code) || /timeout|SQLITE_BUSY|SQLITE_LOCKED/i.test(msg)) {
    return new CapabilityError(code === '57014' ? 'DB_TIMEOUT' : 'DB_UNAVAILABLE', msg, { errorClass: 'transient', retryable: true, details: { code } });
  }
  if (code.startsWith('28') || code === '42501') return new CapabilityError('DB_PERMISSION', msg, { errorClass: 'authorisation', retryable: false });
  if (code.startsWith('23') || /constraint/i.test(msg)) return new CapabilityError('DB_CONSTRAINT', msg, { errorClass: 'business', retryable: false, details: { code } });
...
const failureModes = [
  { code: 'DB_UNAVAILABLE', class: 'transient' as const, retryable: true, description: 'Connection failed, deadlock or serialisation failure' },
  { code: 'DB_TIMEOUT', class: 'transient' as const, retryable: true },
  { code: 'DB_PERMISSION', class: 'authorisation' as const, retryable: false },
  { code: 'DB_CONSTRAINT', class: 'business' as const, retryable: false, description: 'A constraint rejected the change' },
 ...
 */
export function createDatabaseCapabilities(config: AdapterConfig, opts: { pgFactory?: PgFactory } = {}): CapabilityAdapter[] {
  const drivers = new Map<string, DbDriver>();

  const driverFor = (name: string): DbDriver => {
    const url = config.datasources[name];
    if (!url) {
      throw new CapabilityError('DB_UNKNOWN_DATASOURCE', `Datasource '${name}' is not configured (available: ${Object.keys(config.datasources).join(', ') || 'none'})`, {
        errorClass: 'contract',
        retryable: false,
      });
    }
    ...
    if (!d) {
      d = url.startsWith('sqlite:') ? sqliteDriver(url.replace(/^sqlite:(\/\/)?/, '')) : postgresDriver(url, opts.pgFactory ?? defaultPgFactory);
      drivers.set(name, d);
    }
    return d;
  };
  ...
  const query: CapabilityAdapter = {
    declaration: {
      ...
      outputSchema: {
        ...
        properties: { rows: { type: 'array', items: { type: 'object' } }, rowCount: { type: 'integer' }, truncated: { type: 'boolean' } },
        additionalProperties: false,
      },
      effect: 'idempotent',
      scopes: ['db:read'],
    ...
    async execute(ctx, input: { datasource: string; sql: string; params?: SqlParam[]; maxRows?: number }) {
      if (!SINGLE_STATEMENT.test(input.sql) || !READ_ONLY_START.test(input.sql)) {
        throw new CapabilityError('DB_INVALID_STATEMENT', 'database-query accepts a single SELECT/WITH/VALUES statement; pass literals as params', { errorClass: 'contract', retryable: false });
      }
      try {
        return await driverFor(input.datasource).query(input.sql, input.params ?? [], { maxRows: input.maxRows ?? 1000, timeoutMs: 30_000 });
      } catch (e) {
  ...
  const command: CapabilityAdapter = {
    declaration: {
      ...
      version: '1.0.0',
      family: 'database',
      description: 'Run a parameterised INSERT/UPDATE/DELETE. Effectful: requires an idempotencyKey, which is applied in the same transaction so the change happens exactly once.',
      inputSchema: inputSchema({}),
      outputSchema: {
        type: 'object',
        required: ['rowCount', 'rows', 'duplicate'],
        properties: { rowCount: { type: 'integer' }, rows: { type: 'array', items: { type: 'object' } }, duplicate: { type: 'boolean' } },
        additionalProperties: false,
      },
      effect: 'effectful',
      scopes: ['db:write'],
    ...
    async execute(ctx, input: { datasource: string; sql: string; params?: SqlParam[] }) {
      if (!SINGLE_STATEMENT.test(input.sql)) {
        throw new CapabilityError('DB_INVALID_STATEMENT', 'database-command accepts a single statement; pass literals as params', { errorClass: 'contract', retryable: false });
      }
      try {
        return await driverFor(input.datasource).command(input.sql, input.params ?? [], { idempotencyKey: ctx.idempotencyKey, timeoutMs: 30_000 });
      } catch (e) {
        if (ctx.signal.aborted) throw e;
        throw classify(e);
      }
