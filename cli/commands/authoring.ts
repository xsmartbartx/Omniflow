...
import { flag, has, UsageError } from '../args.ts';
...
function summarise(ctx: CliContext, draft: any): string[] {
  ...
  const lines = [
    `${v.ok ? s.green('✓ valid') : s.red('✗ not valid yet')}${v.risk ? `  risk ${v.risk.level} (${v.risk.score})` : ''}  ${s.dim(draft.id)}`,
  ];
  for (const e of (v.errors ?? []).slice(0, 8))
    lines.push(`  ${s.red('error')}${s.dim(`[${e.code}]`)} ${e.path ? `${e.path}: ` : ''}${e.message}`);
  for (const f of (v.risk?.findings ?? []).filter((x: any) => x.blocking))
    lines.push(`  ${s.red('blocking')}${s.dim(`[${f.ruleId}]`)} ${f.message}`);
  return lines;
}
...
export async function planCommand(ctx: CliContext): Promise<number> {
  ...
  const s = ctx.style;
  const r = await client(ctx).post('/v1/authoring/plan', {
    intent,
    ...(flag(ctx.args, 'workflow') ? { workflow: flag(ctx.args, 'workflow') } : {}),
  });
  emit(ctx, r, () => {
    const out: string[] = [];
    if (r.plan.rationale) out.push(s.bold('Rationale'), r.plan.rationale, '');
    if (r.plan.openQuestions?.length)
      out.push(s.bold('Open questions'), ...r.plan.openQuestions.map((q: string) => `  - ${q}`), '');
    if (r.plan.injectionSignals?.length)
      out.push(
        s.yellow(
          `Warning: ${r.plan.injectionSignals.length} prompt-injection signal(s) in the supplied material. Review the draft with extra care.`,
        ),
        '',
      );
    if (r.mode === 'proposal-only')
      out.push(
        s.yellow('This workflow is at autonomy tier T0 (advisory): nothing was saved.'),
        '',
        r.plan.manifest ?? '',
      );
    else if (r.draft)
      out.push(
        ...summarise(ctx, r.draft),
        '',
        `Review it with: omniflow drafts show ${r.draft.id}   ·   publish with: omniflow drafts submit ${r.draft.id}`,
      );
      ...
      r.items.length === 0
        ...
        : table(
            r.items.map((d: any) => [
              ...
              d.origin,
              d.status,
              d.validation?.ok ? s.green('valid') : s.red('invalid'),
              ago(d.updatedAt),
            ]),
            ['DRAFT', 'WORKFLOW', 'ORIGIN', 'STATUS', 'VALIDATION', 'UPDATED'],
            s,
          ),
    );
    return 0;
  ...
  switch (sub) {
    case 'show': {
      ...
      if (has(ctx.args, 'manifest') && !ctx.json) {
        ...
        emit(ctx, d, () =>
          [
            `${s.bold(d.workflowName ?? '(unnamed)')}  ${d.origin} · ${d.status}`,
            ...summarise(ctx, d),
            ...(d.notes?.rationale ? ['', d.notes.rationale] : []),
            ...(d.notes?.openQuestions ?? []).map((q: string) => `  ? ${q}`),
            '',
            d.manifestText,
          ].join('\n'),
        );
    ...
    case 'submit': {
      const r = await api.post(
        `${path}/submit`,
        has(ctx.args, 'canary') ? { canaryPercent: Number(flag(ctx.args, 'canary')) } : {},
      );
      emit(ctx, r, () =>
        r.status === 'published'
          ? `${s.green('✓')} published ${s.bold(`${r.version.name}@${r.version.version}`)}`
          : `${s.yellow('…')} needs approval (${r.decision.reason}) — change ${r.change.id}`,
      );
      return 0;
    }
    case 'apply': {
      ...
      emit(ctx, r, () =>
        r.applied
          ? `${s.green('✓')} tier ${r.tier}: ${r.status === 'published' ? `published ${r.version}` : `change request ${r.changeId} opened for approval`}\n  ${r.verdict.reason}`
          : `${s.yellow('—')} tier ${r.tier}: nothing applied. ${r.verdict.reason}`,
      );
      return 0;
    }
    case 'delete': {
    ...
    default:
      throw new UsageError(
        `Unknown drafts subcommand '${sub}'. Try: list, create, show, validate, submit, apply, delete`,
      );
  }
}
...
export async function importCommand(ctx: CliContext): Promise<number> {
  ...
  if ((kind !== 'crontab' && kind !== 'script') || !file)
    throw new UsageError(
      'Usage: omniflow import crontab <file|-> [--owner e] [--timezone tz] [--out-dir dir]\n       omniflow import script <file> [--name n]',
    );
  const s = ctx.style;
  const api = client(ctx);
  ...
  if (kind === 'script') {
    const r = await api.post('/v1/import/script', {
      script: text,
    ...
    emit(ctx, r, () =>
      [
        r.plan.rationale ?? '',
        ...(r.plan.openQuestions ?? []).map((q: string) => `  ? ${q}`),
        ...(r.draft ? summarise(ctx, r.draft) : [s.red('No usable draft was produced.')]),
      ].join('\n'),
    );
    return r.plan.ok ? 0 : 1;
  }

  const r = await api.post('/v1/import/crontab', {
    text,
    ...(flag(ctx.args, 'owner') ? { owner: flag(ctx.args, 'owner') } : {}),
    ...(flag(ctx.args, 'timezone') ? { timezone: flag(ctx.args, 'timezone') } : {}),
  });
  ...
  if (dir) {
    ...
    for (const d of r.drafts) {
      ...
      if (d.script)
        writeFileSync(join(resolve(ctx.cwd, dir), `${d.draft.workflowName}.sh`), d.script.content, { mode: 0o644 });
    }
  }
  emit(ctx, r, () => {
    const out = r.drafts.map(
      (d: any) =>
        `${s.bold(d.draft.workflowName ?? '?')}  ${d.draft.validation?.ok ? s.green('valid') : s.red('invalid')}  ${s.dim(`line ${d.line}`)}  ${s.dim(d.draft.id)}\n${d.notes.map((n: string) => `    - ${n}`).join('\n')}`,
    );
    for (const k of r.skipped) out.push(`${s.yellow('skipped')} line ${k.line}: ${k.reason}`);
    if (dir)
      out.push(
        '',
        `${s.green('✓')} wrote manifests${r.drafts.some((d: any) => d.script) ? ' and suggested scripts' : ''} to ${dir}`,
      );
    return out.join('\n');
  });
  return 0;
...
export async function explainCommand(ctx: CliContext): Promise<number> {
  ...
  if ((kind !== 'workflow' && kind !== 'run') || !target)
    throw new UsageError('Usage: omniflow explain workflow <name> [--version v]\n       omniflow explain run <run-id>');
  const api = client(ctx);
  if (kind === 'workflow') {
    const e = await api.get(
      `/v1/workflows/${encodeURIComponent(target)}/explain${flag(ctx.args, 'version') ? `?version=${encodeURIComponent(flag(ctx.args, 'version')!)}` : ''}`,
    );
...
export async function docsCommand(ctx: CliContext): Promise<number> {
  ...
  if (kind === 'capabilities') text = await api.getText('/v1/docs/capabilities');
    ...
    throw new UsageError(
      'Usage: omniflow docs workflow <name> [--format markdown|mermaid] [--out file]\n       omniflow docs capabilities|index [--out file]',
    );
