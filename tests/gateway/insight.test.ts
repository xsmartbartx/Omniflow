...
const failStep = (id: string, extra: Record<string, unknown> = {}) => ({ id, type: 'capability', uses: 'util-fail@^1', with: { errorClass: 'business', message: 'nope' }, ...extra });
...
async function runMany(workflow: string, n: number): Promise<void> {
  ...
  await until(() => api.app.state.runs.listRuns({ tenant: 'default', workflow, limit: 100 }).every((r) => ['succeeded', 'failed'].includes(r.status)) && api.app.state.runs.countRuns({ tenant: 'default', workflow }) >= n);
}
...
describe('insight endpoints', () => {
  ...
  it('runs the analysis agent on demand and manages the proposal queue', async () => {
    ...
    await admin.post('/v1/workflows', { manifest: yamlWf('swallower', [echoStep('first', 1), failStep('flaky', { dependsOn: ['first'], onError: 'continue' })]) });
    ...
    expect((await admin.get('/v1/proposals?status=open&workflow=swallower')).body.items.some((x: { id: string }) => x.id === p.id)).toBe(false);
    ...
    const audit = (await admin.get('/v1/audit/events?type=insight.analysis-completed,agent.proposal-created')).body.items;
...
describe('alert configuration', () => {
  ...
  it('accepts alert channels that are defined, and rejects ones that are not', () => {
    const dir = api.dir;
    const ok = loadConfig({ ...base, OMNIFLOW_DATA_DIR: dir, OMNIFLOW_CHANNELS: '{"ops":"slack:https://hooks.slack.com/x"}', OMNIFLOW_ALERT_CHANNELS: 'ops' }, { cwd: dir });
    expect(ok.alertChannels).toEqual(['ops']);
    expect(() => loadConfig({ ...base, OMNIFLOW_DATA_DIR: dir, OMNIFLOW_ALERT_CHANNELS: 'nowhere' }, { cwd: dir })).toThrow(/not defined in OMNIFLOW_CHANNELS/);
    ...
    expect(() => loadConfig({ ...base, OMNIFLOW_DATA_DIR: dir, OMNIFLOW_ALERT_INTERVAL_SECONDS: '1' }, { cwd: dir })).toThrow(/between 5 and 3600/);
  });
});
...
describe('alerts reach the operator\'s chat channel', () => {
  it('delivers a raised and a resolved alert through the configured webhook', async () => {
    ...
    const wired = await makeApi({ env: { OMNIFLOW_CHANNELS: JSON.stringify({ ops: `generic:http://127.0.0.1:${port}/hook` }), OMNIFLOW_ALERT_CHANNELS: 'ops', OMNIFLOW_ALLOW_PRIVATE_EGRESS: 'true' } });
    try {
      ...
      await until(() => wired.app.state.runs.listRuns({ tenant: 'default', workflow: 'breaks' }).filter((r) => r.status === 'failed').length === 5);
      ...
      expect((await a.get('/v1/insights/alerts')).body.active[0]).toMatchObject({ key: 'workflow-failing:breaks', severity: 'critical' });
  ...
  it('keeps working when the channel is unreachable', async () => {
    const wired = await makeApi({ env: { OMNIFLOW_CHANNELS: JSON.stringify({ ops: 'generic:http://127.0.0.1:1/hook' }), OMNIFLOW_ALERT_CHANNELS: 'ops', OMNIFLOW_ALLOW_PRIVATE_EGRESS: 'true' } });
    try {
      const a = await wired.login();
      await a.post('/v1/workflows', { manifest: yamlWf('breaks', [failStep('x')]) });
      for (let i = 0; i < 5; i++) await a.post('/v1/workflows/breaks/run', {});
