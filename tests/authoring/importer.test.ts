...
const check = (manifest: string) => {
  ...
  return { ...r, risk: r.plan ? analyzeWorkflow({ manifest: parse(manifest), plan: r.plan, origin: 'human' }) : undefined };
};

describe('tokenize', () => {
  it('splits on whitespace and honours quotes and escapes', () => {
    expect(tokenize(`curl -s "https://x.test/a b" 'it''s' plain\\ word`)).toEqual(['curl', '-s', 'https://x.test/a b', 'its', 'plain word']);
...
describe('needsShell', () => {
  it('spots real shell syntax and ignores quoted text and URLs', () => {
    for (const c of ['a | b', 'a > out', 'a && b', 'a; b', 'ls *.log', 'echo $HOME', 'echo "$HOME"', 'echo `date`', 'echo $(date)', 'cat ~/x']) expect(needsShell(c), c).toBe(true);
    for (const c of ['curl -s https://x.test/y?a=1&b=2', "grep 'a|b' file", 'echo "a > b"', '/usr/bin/backup --full', "echo '$HOME'"]) expect(needsShell(c), c).toBe(false);
  });
});
...
describe('crontab import (Lift)', () => {
  it('turns a plain job into a compiling workflow with a schedule and a sunset date', () => {
    ...
    expect(m.steps[0]).toMatchObject({ id: 'run', uses: 'shell-exec@^1', with: { argv: ['/usr/bin/curl', '-fsS', 'https://backup.example.com/run'] }, sunset: '2026-08-30' });
  ...
  it('understands macros, comments, blank lines, environment lines and MAILTO', () => {
    const r = importCrontab(['# nightly jobs', '', 'MAILTO=ops@example.com', 'PATH=/usr/bin:/bin', '@daily /usr/bin/curl -s https://a.example.com/x', '@hourly /usr/bin/curl -s https://b.example.com/y'].join('\n'), opts);
  ...
  it('warns about credentials and destructive commands', () => {
    const r = importCrontab(['0 4 * * * /usr/bin/curl -u admin:hunter2 https://x.example.com', '0 5 * * * /usr/bin/curl https://user:pw@x.example.com/a', '0 6 * * * rm -rf /var/cache/app/*', '0 7 * * * /opt/job.sh --password=abc123'].join('\n'), opts);
  ...
  it('skips what it cannot express, and says why', () => {
    const r = importCrontab(['@reboot /usr/bin/start-thing', 'not a crontab line at all', '* * * * *', '0 0 * * * echo "unbalanced', '@sometimes /bin/true', '61 0 * * * /bin/true'].join('\n'), opts);
  ...
  it('gives duplicate jobs distinct names', () => {
    const r = importCrontab('0 1 * * * /usr/bin/curl https://a.example.com/x\n0 2 * * * /usr/bin/curl https://a.example.com/x\n0 3 * * * /usr/bin/curl https://a.example.com/x', opts);
  ...
  it('carries the timezone, team and sunset settings through', () => {
    const [w] = importCrontab('0 9 * * * /usr/bin/curl https://a.example.com/x', { ...opts, timezone: 'Europe/Warsaw', team: 'platform', sunsetDays: 30 }).workflows;
    const m = parse(w!.manifest);
