...
describe('the documented extension path: a custom distribution with its own capability', () => {
  ...
  beforeAll(async () => {
    ...
    const config = loadConfig({ OMNIFLOW_DATA_DIR: dir, OMNIFLOW_ADMIN_PASSWORD: 'correct-horse-battery-staple', OMNIFLOW_LOG_LEVEL: 'silent', OMNIFLOW_ENV: 'development' }, { cwd: dir });
    ...
    await server.ready();
    const res = await server.inject({ method: 'POST', url: '/v1/auth/login', payload: { email: 'admin@omniflow.local', password: 'correct-horse-battery-staple' } });
    const cookie = String(res.headers['set-cookie']).split(';')[0]!;
    const call = async (method: string, url: string, body?: unknown) => {
      const r = await server.inject({ method: method as never, url, headers: { cookie, 'x-requested-with': 'omniflow', ...(body ? { 'content-type': 'application/json' } : {}) }, ...(body ? { payload: JSON.stringify(body) } : {}) });
      return { status: r.statusCode, body: r.body ? JSON.parse(r.body) : null };
    };
    admin = { get: (u: string) => call('GET', u), post: (u: string, b?: unknown) => call('POST', u, b ?? {}) } as unknown as Client;
  });
  afterAll(async () => {
    await server.close();
  ...
  it('registers as a plugin with a named owner, next to the built-ins', async () => {
    const caps = (await admin.get('/v1/capabilities')).body.items as Array<{ name: string; source: string; owner: string }>;
    expect(caps.find((c) => c.name === 'acme-lookup-customer')).toMatchObject({ source: 'plugin', owner: 'crm-platform-team' });
    expect(caps.some((c) => c.name === 'util-echo')).toBe(true);
  });

  it('is validated, run and failure-classified like any built-in', async () => {
    const wf = yamlWf('customer-tier', [{ id: 'find', type: 'capability', uses: 'acme-lookup-customer@^1', with: { customerId: '${{ inputs.id }}' } }], {
    ...
    const missing = (await admin.post('/v1/workflows/customer-tier/run', { inputs: { id: 'c_999' } })).body.run.id as string;
    ...
    const bad = yamlWf('bad-customer', [{ id: 'find', type: 'capability', uses: 'acme-lookup-customer@^1', with: { customerId: 'not-an-id', extra: 1 } }]);
  ...
  it('can be unit-tested on its own, with no platform at all', async () => {
    ...
    await expect(lookupCustomer.execute(ctx, { customerId: 'c_0' })).rejects.toMatchObject({ code: 'CUSTOMER_NOT_FOUND' });
  });
});
