...
const walk = (dir: string): string[] =>
  readdirSync(join(ROOT, dir)).flatMap((f) =>
    statSync(join(ROOT, dir, f)).isDirectory() ? walk(join(dir, f)) : [join(dir, f)],
  );

describe('configuration documentation cannot drift from the code', () => {
  const documented = new Set(
    [...read('.env.example').matchAll(/\b((?:OMNIFLOW|ANTHROPIC)_[A-Z_]+)\b/g)].map((m) => m[1]!),
  );
  const sources = ['server', 'gateway', 'capabilities', 'insight', 'authoring']
    .flatMap((d) => walk(d))
    .filter((f) => f.endsWith('.ts'));
  const used = new Set(
    sources.flatMap((f) => [...read(f).matchAll(/\b(OMNIFLOW_[A-Z_]+|ANTHROPIC_API_KEY)\b/g)].map((m) => m[1]!)),
  );
...
describe('Kubernetes manifests', () => {
  ...
  it('every secret the deployment expects is listed in the header instructions', () => {
    ...
    for (const v of ['OMNIFLOW_MASTER_KEY', 'OMNIFLOW_ADMIN_PASSWORD', 'OMNIFLOW_METRICS_TOKEN'])
...
describe('example workflows and policies', () => {
  ...
  it('the optional examples need something the stock install lacks — and are valid once it is there', async () => {
    ...
    const code = await runCli(
      ['validate', join(ROOT, 'workflows/optional')],
      { out: (t) => (out += t), err: () => {}, readStdin: async () => '' },
      {},
      ROOT,
    );
  ...
  it('seeding publishes the examples on first start, and they run', async () => {
    const api = await makeApi({
      env: {
        OMNIFLOW_SEED_EXAMPLES: 'true',
        OMNIFLOW_WORKFLOWS_DIR: join(ROOT, 'workflows'),
    ...
    try {
      ...
      const hello = (await admin.post('/v1/workflows/hello-world/run', { inputs: { name: 'Ada' } })).body.run
        .id as string;
      const router = (
        await admin.post('/v1/workflows/order-router/run', {
          inputs: {
            orders: [
              { id: 'a', total: 20 },
              { id: 'b', total: 5000 },
      ...
      await until(
        () =>
          ['hello', 'router'].length > 0 &&
          [hello, router].every((id) => api.app.state.runs.getRun(id)?.status === 'succeeded'),
      );
      expect((await admin.get(`/v1/runs/${hello}`)).body.run.outputs).toEqual({
        greeting: 'Hello, Ada!',
        loud: 'HELLO, ADA!',
      });
      ...
      const gated = (
        await admin.post('/v1/workflows/approval-gated-refund/run', { inputs: { orderId: 'A-1', amount: 25 } })
      ).body.run.id as string;
      await until(() => api.app.state.runs.getRun(gated)?.status === 'waiting-approval');
      const tooBig = (
        await admin.post('/v1/workflows/approval-gated-refund/run', { inputs: { orderId: 'A-2', amount: 50000 } })
      ).body.run.id as string;
