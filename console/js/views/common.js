...
export const wfLink = (name, version) =>
  h('a', { href: `#/workflows/${enc(name)}` }, name, version ? h('span', { class: 'muted' }, ` @${version}`) : null);
export const runLink = (id) =>
  h('a', { href: `#/runs/${enc(id)}`, class: 'mono', title: id }, id.replace(/^run_/, '').slice(-10));
      ...
      ...(showWorkflow ? [{ label: 'Workflow', render: (r) => wfLink(r.workflow, r.version) }] : []),
      {
        ...
        render: (r) =>
          h(
            ...
            r.dryRun ? h('span', { class: 'muted' }, ' dry run') : null,
            r.canary ? h('span', { class: 'muted' }, ' canary') : null,
          ),
      },
  ...
  return h(
    ...
    triggers.length
      ? triggers.map((t) =>
          h(
            'span',
            { class: 'badge badge-neutral', title: t.nextFireAt ? `next: ${timestamp(t.nextFireAt)}` : t.name },
            t.type,
          ),
        )
      : h('span', { class: 'muted' }, 'manual'),
  );
}
