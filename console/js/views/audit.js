...
import { button, card, codeBlock, dataTable, input, notice, openDialog, pageHeader, toast } from '../ui.js';
import { runLink } from './common.js';

export default async function audit() {
  const type = input({
    placeholder: 'Event type (e.g. run.failed, workflow.published)',
  ...
  const load = async (append) => {
      ...
      dataTable({
        ...
        onRow: (e) =>
          openDialog({
            title: `#${e.seq} ${e.type}`,
            wide: true,
            body: codeBlock(JSON.stringify(e, null, 2)),
    ...
    clear(more).append(
      r.items.length === 100
        ? button('Load older', { onClick: () => load(true) })
        : h('span', { class: 'muted' }, `${rows.length} of ${r.total} events shown`),
    );
  };
  let debounce;
  ...
  const verify = button('Verify integrity', {
    ...
    onClick: async () => {
      ...
      clear(verifyBox).append(
        r.ok
          ? notice(
              'good',
              h('strong', {}, 'The audit log is intact. '),
              `${r.checked} events verified; every hash and link matches.`,
            )
          : notice(
              'bad',
              h('strong', {}, 'Tampering detected. '),
              `The chain breaks at event ${r.brokenAtSeq}: ${r.reason}. Treat this as a security incident.`,
            ),
      );
  ...
  return h(
    'div',
    {},
    pageHeader('Audit log', 'An append-only, hash-chained record of everything that happened.', [
      verify,
      can('audit.read')
        ? h('a', { class: 'btn', href: '/v1/audit/export', download: 'omniflow-audit.ndjson' }, 'Export')
        : null,
    ]),
    h(
      'div',
