...
import { clearSession, can, loadSession, session } from './session.js';
...
const ROUTES = [
  ...
  { path: '/dashboard', title: 'Dashboard', nav: ['Overview', 'dashboard'], view: () => import('./views/dashboard.js') },
  ...
  { path: '/editor', title: 'Editor', nav: ['Build', 'authoring', 'Editor & AI'], view: () => import('./views/editor.js') },
  ...
  { path: '/approvals', title: 'Approvals', nav: ['Operate', 'approvals', null, 'approvals'], view: () => import('./views/approvals.js') },
  { path: '/changes', title: 'Change requests', nav: ['Operate', 'changes', null, 'changes'], view: () => import('./views/changes.js') },
  { path: '/insights', title: 'Insights', nav: ['Operate', 'insight', null, 'alerts'], view: () => import('./views/insights.js') },
  { path: '/capabilities', title: 'Capabilities', nav: ['Configure', 'capability'], view: () => import('./views/capabilities.js') },
  ...
  { path: '/admin', title: 'Users & keys', nav: ['Govern', 'admin'], can: 'user.manage', view: () => import('./views/admin.js') },
...
function buildShell() {
  ...
  for (const r of ROUTES.filter((x) => x.nav && (!x.can || can(x.can)))) {
    ...
    nav.append(h('a', { href: `#${r.path}`, dataset: { path: r.path } }, icon(ic), h('span', {}, label ?? r.title), badgeKey ? h('span', { class: 'count', dataset: { badge: badgeKey }, hidden: true }, '0') : null));
  }
  const me = session.me;
  const side = h(
    'aside',
    { class: 'side', id: 'side' },
    h('div', { class: 'brand' }, h('img', { src: '/favicon.svg', alt: '' }), 'OmniFlow', h('span', { class: ['env-pill', me.environment] }, me.environment)),
    nav,
    h('div', { class: 'side-foot' }, h('div', { class: 'who' }, me.user?.name ?? me.principal.name), h('div', { class: 'roles' }, me.principal.roles.join(', ')), h('a', { href: '#/account', class: 'row' }, icon('admin', { size: 14 }), 'Account'), h('a', { href: '#/login', class: 'row', onClick: signOut }, icon('logout', { size: 14 }), 'Sign out')),
  );
...
async function renderRoute() {
  ...
  if (!match.route || (match.route.can && !can(match.route.can))) {
    ...
    return clear(main).append(errorBox(new Error(match.route ? 'You do not have access to this page.' : 'That page does not exist.'), { title: 'Not found' }), h('p', {}, h('a', { href: '#/dashboard' }, 'Back to the dashboard')));
  }
